...
impl utils::Solution for Day {
    ...
    fn part2(&self, input: &str) -> String {
        ...
        let mut total = 0;
        for line in input {
            let unique: Vec<&str> = line.split("|").nth(0).unwrap().trim().split(" ").collect();
            ...
            let num3 = unique
                ...
                .filter(|x| x.len() == 5)
                .filter(|x| {
                    x.contains(num1.chars().nth(0).unwrap())
            ...
            let b = num4.split("").filter(|x| !num3.contains(x)).nth(0).unwrap();
            ...
            let nums = vec![num0, num1, num2, num3, num4, num5, num6, num7, num8, num9];
            ...
            let output = line
                ...
                .map(|digit| {
                    for x in 0..=9 {
                        if digit.len() != nums[x].len() {
                            continue;
                        }
                        if digit.chars().filter(|&y| nums[x].contains(y)).count() == digit.len() {
                            return char::from_digit(x as u32, 10).unwrap();
                        }
                    }
                    unreachable!()
                })
            ...
            total += u32::from_str_radix(&output, 10).unwrap();
        }
        total.to_string()
    }
}
